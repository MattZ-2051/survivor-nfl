...
<div on:mouseenter={() => (active = item)} on:mouseleave={() => (active = null)} class="relative">
	<Motion let:motion transition={{ duration: 0.3 }}>
		<p
			use:motion
			class="cursor-pointer text-fifth-color hover:opacity-[0.9] hover:cursor-pointer hover:scale-110"
		>
			{item}
		</p>
	</Motion>
				...
				{#if active === item}
					<div class="absolute left-1/2 -translate-x-1/2 transform pt-4">
						<Motion let:motion {transition} layoutId="active">
							<div
								use:motion
								class="overflow-hidden rounded-2xl border border-gray-500 shadow-xl backdrop-blur-sm"
							>
								<Motion let:motion layout>
									<div use:motion class="h-full w-max p-4 bg-first-color border border-gray-500">
										<slot />
									</div>
								</Motion>
							</div>
